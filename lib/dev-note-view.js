...
import DevNoteFinderView from './dev-note-finder-view'
...
export default class DevNoteView {
  ...
  constructor(devNoteData) {
    ...
    this.items = [];
    this.devNoteView = new DevNoteFinderView(this.devNoteData);
    ...
    this.selectList = new SelectListView(
      {
        ...
        didConfirmSelection: (note) => {
          ...
          if(this.delete_note_after){
            ...
            if(markers.length){
              this.devNoteView.show(note)
            } else {
              atom.notifications.addInfo("Note location not found in open files.")
            }
